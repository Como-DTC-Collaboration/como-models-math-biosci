...
Epidemiological differential equations can be solved straightforwardly using simple fixed
...
solver and various heuristic techniques. Adaptive solvers are able to vary the step
...
where $g$ is discontinuous present further challenges to both adaptive and fixed step size methods, whose error properties (and, for adaptive step size methods, the algorithms used to determine the time steps) typically assume differentiability of $g$.
...
of 0.5 days, once without any consideration of the discontinuities and once
with solver time steps inserted at the discontinuities (method 1 above). These 
results are compared to an accurate (but potentially inefficient) solution
obtained using the LSODA solver.
...
times <- seq(0, 20, by = 1)
df1 <- run(model, times, solve_method="lsoda")$changes
df1$method = "Accurate (LSODA)"

solver_times <- seq(0, 20, by=0.5)
df2 <- run(model, solver_times, solve_method="euler")$changes

df2_dense <- data.frame(time=NULL, value=NULL, compartment=NULL)
for (t in times) {
  cases = subset(df2, t-1<df2[,1] & df2[,1]<=t)
  incidence = subset(cases, cases[,3]=="Incidence")
  death = subset(cases, cases[,3]=="Deaths")
  df2_dense = rbind(df2_dense, 
                        data.frame(time=t,
                                   value=sum(incidence$value),
                                   compartment="Incidence",
                                   age_range="0-150"))
...
df2_dense$method = "Euler, dt=0.5 day"



solver_times <- sort(c(solver_times, 4.75, 9.75))
df3 <- run(model, solver_times, solve_method="euler")$changes

df3_dense <- data.frame(time=NULL, value=NULL, compartment=NULL)
for (t in times) {
  cases = subset(df3, t-1<df3[,1] & df3[,1]<=t)
  incidence = subset(cases, cases[,3]=="Incidence")
  death = subset(cases, cases[,3]=="Deaths")
  df3_dense = rbind(df3_dense, 
                        data.frame(time=t,
                                   value=sum(incidence$value),
                                   compartment="Incidence",
                                   age_range="0-150"))
...
df3_dense$method = "Euler, dt=0.5 day, start/stop at discon"

df <- rbind(df1, df3_dense, df2_dense)

ggplot(subset(df, df[,3]=="Incidence"), 
       aes(x = time, y = value, fill = method)) +
  geom_bar(stat="identity", position = position_dodge()) +
...
additional error caused by the discontinuous $\delta_V$ is apparent.
...
times <- seq(0, 20, by = 1)
df2 <- run(model, times, solve_method="lsoda")$changes
