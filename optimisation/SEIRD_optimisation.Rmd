...
$$ C_t^* \sim \text{Pois}(C_t), $$
$$ D_t^* \sim \text{Pois}(D_t). $$
...
$$
where $C_t$ and $D_t$ are both functions of the transmission parameters. Note that, in the above, we have dropped the constant of proportionality for readability.
...
where $C(t)$ and $D(t)$ are cases and deaths from the raw ODE model outputs respectively, while $C^*(t)$ and $D^*(t)$ are noisy cases and deaths of the SEIRD model.
