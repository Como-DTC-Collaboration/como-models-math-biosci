...
We first generate case and deaths series which qualitatively replicates the London COVID-19 data series which we investigate above. The initial conditions and transmission parameters used to generate the synthetic data are as follows 
...
simulating_para <- list(beta=9.1e-1, kappa=8.7e-1, gamma=5.3e-1, mu=9.7e-2,
						S0=9.999988e-1, E0=1e-7, I0=1e-7, R0=1e-6)
...
beta_range <- c(seq(simulating_para$beta, max(0.01, simulating_para$beta - 0.5),
					by = -0.02),
				seq(simulating_para$beta, simulating_para$beta + 0.5, by = 0.02))
gamma_range <- c(seq(simulating_para$gamma,
                     max(0.01, simulating_para$gamma - 0.5),
                     by = -0.02),
                 seq(simulating_para$gamma,
                     simulating_para$gamma + 0.5, by = 0.02))
...
The actual parameters value for each is indicated by the vertical line. 
...
Here, we construct the profile likelihood by adding more parameters for optimisation.
We will see how the amount of parameters affect the identifiability of the transmission parameters of the SEIRD model.
...
mu_range <- c(seq(simulating_para$mu,
                  ...
                  by = -0.005),
              seq(simulating_para$mu, simulating_para$mu + 0.1, by = 0.005))
...
However, $\mu$ has better identifiability than the remaining parameters.
...
With more data points, the identifiability of $\gamma$ improves. $\gamma$ and $\mu$ are thought to be identifiable up to a certain range, the log-likelihood values decreases when both parameters are smaller than a certain value. 
