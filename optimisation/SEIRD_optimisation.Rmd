...
These values are chosen from the optimisation of the SEIRD model on all parameters, including initial conditions, which is done without showing it in this vignette.
...
These characteristics greatly simplify the task of estimating $R_0$, since the models used to fit it require fewer assumptions.
There is thus a tension between waiting to obtain more data and restricting the data used for estimation to avoid having to make more assumptions about transmission processes.
Our maximum likelihood fit had an $R_0$ value of X, which is comparable to other values in the literature, for instance $R_0 = 2.4$ was used in Imperial College's Report 9 [cite Ferguson report 9 paper].
This illustrates that even relatively simple models like the SEIRD may be used to yield quick and potentially useful estimates of quantities of interest.
...
The transmission parameters, on the other hand, are taken to be the parameter set which yielded the maximum log-likelihood value from the optimisations on real data.
Here, we assume that we know the exact population size.
The population size is set to the population size of London.
...
The synthetic data is extracted to have the same amount of time points (days) as the London COVID-19 data. 
...
The profile likelihood is then constructed for the transmission parameters, while the initial conditions are fixed at their true values.

## Profile likelihood of $\beta$ and $\gamma$

We first suppose that only $\beta$ and $\gamma$ are unknown and apply the profile likelihood approach to $\beta$ and $\gamma$ only, fixing the other transmission parameters ($\kappa$ and $\mu$) at their known values.
So when considering $\beta$, we scan across a range of different values from this parameter and perform optimisation to find the $\gamma$ value which maximises the log-likelihood for that particular $\beta$ value; when considering $\gamma$, we perform optimisation for $\beta$.
