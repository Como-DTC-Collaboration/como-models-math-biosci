...
To further illustrate this, we now plot the estimated parameters value, $R_0$ value and log-likelihood value from each of the replicates for each of the four model parameters.
...
All optimisation runs return a wide range of $R_0$ values, which gives different interpretation of the epidemic. 
...
The profile likelihood shows a flat graph for both $\beta$ and $\gamma$.
This implies that the parameters are unidentifiable.
...
beta_plot <- ggplot() + 
  geom_point(data = beta_loglikelihood_values, aes(x=beta, y=likelihood_value)) +
  geom_vline(xintercept = simulating_para$beta) +
  labs(y = "log-likelihood value") + 
  scale_y_continuous(labels = function(x) format(x, scientific = TRUE))
gamma_plot <- ggplot() + 
  geom_point(data = gamma_loglikelihood_values, aes(x=gamma, y=likelihood_value)) +
...
We plot, in the left panel, the log-likelihood values for different values of $\beta$ while fixing the remaining parameters to the true parameter values. 
A similar plot is shown in the right panel for $\gamma$ instead of $\beta$.
The graph shows that there exist an optimal maximum log-likelihood value for the two parameters.
However, when optimising one parameter while fixing the other, the parameters compensate each other and become unidentifiable.   

## Profile likelihood with more information

If more data are collected, it may be possible to better identify a model's parameters, and here we investigate this for the SEIRD model.
...
It is clear that the identifiability of these two parameters are improved with extra data points.
...
These results show that all transmission parameters, except for $\gamma$, are not identifiable since there is not a clear maximum to the profile likelihood in each case.
...
Because of this similarity in the effect of each of these parameters on the outputs, there is a positive correlation in a log-likelihood plot in ($\kappa$, $\gamma$) space, meaning that the model suffers from identifiability issues. 
...
As shown in the profile likelihood graph, the parameters are now identifiable.
