...
These values are chosen from the optimisation of the SEIRD model on all parameters, including initial conditions, which is done in the background, without showing in this vignette.
...
The true parameter value used to generate the data for each is indicated by the vertical lines. 
...
With more data points, the identifiability of $\gamma$ improves. $\gamma$ and $\mu$ are not identifiable for values larger than 0.4 for $\gamma$ and 0.07 for $\mu$. However, the log-likelihood values decrease when both parameters are smaller than the given values, observed from the graph. 
