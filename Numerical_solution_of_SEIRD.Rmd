...
Compartmental epidemiological models (such as SEIR and its variants) are 
_differential equation_ models—that is, given particular values of the 
model parameters, they do not directly specify the output of the model over
time, but rather they specify a system of differential equations whose solution
is the model output over time. Like many differential equation models of
interest in scientific applications, SEIR differential equations do not admit
analytical solutions. Instead, an approximation of the solution must be
...
step, unacceptably high error in the numerical solution may occur. We recommend
more sophisticated adaptive step size solvers when solving SEIR-type models, as
implemented in the comomodels package.
...
$$f_0 = f(t=t_0);$$
$$f_{i+1} = f_i + \Delta t g(t_i, f_i),\:\:\:\: i=1, \dots, N-1.$$
...
## Effect of solver step size on the SEIR model
...
ggplot(subset(all_changes, all_changes[,3]=="Deaths"), 
  ...
  labs(title="Deaths")
```
These results indicate that for the SEIR model with these parameters, a daily
...
valid and tenable models for certain phenomena, we see that a daily SEIR 
difference equation cannot be considered equivalent to the original SEIR
...
1. To obtain an accurate solution, the solver step size must be set to some low
value, but it is often unknown how low this value must be.
2. When the solver step size is set to a low value, the method may be
unpleasantly slow.
...
solver (comomodel's default choice for the solver algorithm).
...
solvers such as LSODA for speed and accuracy.
