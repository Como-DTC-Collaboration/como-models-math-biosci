...
their younger counterparts [@verity2020].
...
hence given to those in the more vulnerable categories (e.g. vaccinate older people
first), as was the case with the Covid vaccination campaign in the UK [@govuk].
...
$\delta_V$ and $\delta_{VR}$ respectively. Another assumption we make is that the
vaccine is always effective as long as the individual does not revert to being
susceptible: therefore, there is no movement between the `V` or `VR` and `I` compartments
in the `SEIRDVAge` model.
...
the mortality parameter $\mu$ is age-dependent as according to [@verity2020]:
```{r}
...
For the parameters of each scenario, we adopt the settings from [@Davies2020].
...
result1_changes %>%
  filter(compartment %in% c("Deaths", "Incidence")) %>%
  group_by(interv_type, time) %>%
  mutate(sum_by_group = sum(value)) %>%
    ggplot(aes(x = time, y = value, fill = as.factor(interv_type))) +
